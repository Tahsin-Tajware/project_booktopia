...
import 'package:booktopia/firestore_service.dart';
...
class _HomeViewState extends State<HomeView> {
  ...
  Future<void> fetchBooks() async {
    try {
      // Replace this with your Firestore collection reference
      QuerySnapshot<Map<String, dynamic>> snapshot =
          await FirebaseFirestore.instance.collection('Books').get();

      // Map the snapshot data to your Book class
      Books = snapshot.docs.map((doc) => Book.fromMap(doc.data())).toList();

      // Set the state to trigger a rebuild with the fetched books
      setState(() {});
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    // Check if books is still null or empty
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          return ListTile(
            ...
            // Add more UI elements based on your Book class
          );
        },
      ),
    );
...
//   List<Book> fetchedBooks = await _firestoreService.getBooks();
//   setState(() {
//     Books = fetchedBooks;
//   });
// }
